...
|--------------|----------|----------------|-------------------------------------------------------------------------------------------------------------------------|
| SMTP         | TCP      | 25             | Used for sending emails between mail servers                                                                            |
| Custom TCP   | TCP      | 3000-10000     | Used by various applications, such as Node.js (3000), Grafana (3000), Jenkins (8080), and custom web applications       |
| HTTP         | TCP      | 80             | Allows unencrypted web traffic. Used by web servers (e.g., Apache, Nginx) to serve websites over HTTP                   |
| HTTPS        | TCP      | 443            | Allows secure web traffic using SSL/TLS                                                                                 |
| SSH          | TCP      | 22             | Secure Shell (SSH) for remote server access                                                                             |
| Custom TCP   | TCP      | 6443           | Kubernetes API server port. Used for communication between kubectl, worker nodes, and the Kubernetes control plane       |
| SMTPS        | TCP      | 465            | Secure Mail Transfer Protocol over SSL/TLS. Used for sending emails securely via SMTP with encryption                    |
...
```bash
Docker - It is used to build and run Docker images.
Docker Commons - It is used to manage Docker containers.
Docker Pipeline - It is used to create a pipeline for Docker.
Docker API - It is used to interact with the Docker API.
docker-build-step - It is used to build a Docker image.
AWS Credentials - It is used to manage AWS credentials.
Pipeline stage view - It is used to visualize the pipeline stages.
Kubernetes - It is used to manage Kubernetes clusters.
Kubernetes CLI - It is used to interact with the Kubernetes API.
Kubernetes Client API - It is used to interact with the Kubernetes client API.
Kubernetes Credentials - It is used to manage Kubernetes credentials.
Config File Provider - It is used to manage configuration files.
Prometheus metrics - It is used to monitor the metrics of the application.
```
...
COnfigure git credentials as "git-creds"
```
...
```bash
AmazonEC2FullAccess, AmazonEKS_CNI_Policy, AmazonEKSClusterPolicy, AmazonEKSWorkerNodePolicy, AWSCloudFormationFullAccess, IAMFullAccess, AmazonVPCFullAccess
```
...
```bash
go to the AWS Console -> IAM -> Users -> Your User -> Security Credentials -> Access Keys -> Create Access Key
...
**6.3. Install Node Exporter (Execute in Monitoring Server VM)**
...
**6.4. Configure Prometheus Plugin Integration**
...
### Step 7. Install Grafana (Execute in Monitoring Server VM)**
7.1 Install Grafana (Execute in Monitoring Server VM)
